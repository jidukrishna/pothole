...
def generate_pothole_simulation(grid_size=100, num_potholes=5, scale_factor=0.95):
    ...
    st.write(f"**Original Volume:** {volume_original:.2f} cubic units")
    st.write(f"**Scaled & Smoothed Volume:** {volume_scaled:.2f} cubic units")
    st.write(f"**cuboid for cutting :** {cuboid} cubic units")
    st.write(f"**cuboid waste :** {round(((cuboid - volume_scaled) / cuboid) * 100,2)} cubic units")
...
num_potholes = st.slider("Number of Potholes", min_value=30, max_value=300, value=5)
scale_factor = st.slider("Scale Factor (%)", min_value=50, max_value=100, value=95) / 100

generate_pothole_simulation(grid_size, num_potholes, scale_factor)
